...
let levelStars = {};
try {
  unlockedLevel = parseInt(localStorage.getItem('pq_unlocked')||'0');
  levelStars = JSON.parse(localStorage.getItem('pq_stars')||'{}');
} catch(e){}
function saveProgress(){
  try{
    localStorage.setItem('pq_unlocked', String(unlockedLevel));
    localStorage.setItem('pq_stars', JSON.stringify(levelStars));
  }catch(e){}
}
...
const player = {
  ...
  onGround:false, facing:1, power:'small', // small, big, fire, star, feather, shield
  invincible:0, starTime:0, lives:3, coins:0, score:0,
  hurtTime:0, crouching:false, runFrame:0, jumping:false,
...
function startLevel(idx){
  ...
  // deep-clone entity arrays because we mutate (remove) during play
  ...
  document.getElementById('levelLabel').textContent = `Wereld ${level.worldNum}-${level.stageNum}`;
  showMsg(level.name, 1600);
  updatePowerIcon();
  try { startMusic(level.worldNum-1); } catch(e){}
  if (level.isBoss) setTimeout(()=>playSfx('bossRoar'), 800);
  // Force an immediate render so the player/level are visible on the very
  // first frame, instead of waiting for the next requestAnimationFrame tick
  // (which could be delayed by up to one extra frame on some browsers/devices).
  draw();
}
...
function updatePowerIcon(){
  const icons = {small:'',big:'🍄',fire:'🔥',star:'⭐',feather:'🪶',shield:'🛡️'};
  const el = document.getElementById('powerIcon');
  if (icons[player.power]) {
    el.style.display='none'; // using emoji text instead below
  }
  const hearts = document.getElementById('hearts');
  const label = {small:'Klein 🙂',big:'Groot 🍄',fire:'Vuur 🔥',star:'STER ⭐',feather:'Veer 🪶',shield:'Schild 🛡️'}[player.power];
...
function updatePlayer(){
  ...
  // goal check
  if (player.x > level.goal.x && !level.isBoss){
    winLevel();
  }
  if (level.isBoss && level.boss && !level.boss.alive && player.x > level.boss.x){
    winLevel();
  }
}
...
function killPlayer(){
  ...
  if (player.power!=='small'){
    ...
    floatingTexts.push({x:player.x,y:player.y,text:'Au!',life:40,vy:-1});
    playSfx('hurt');
    return;
  }
  player.lives--;
...
function winLevel(){
  ...
  saveProgress();
  document.getElementById('winStats').innerHTML = `Munten: ${player.coins} 🪙 &nbsp;|&nbsp; Score: ${player.score} &nbsp;|&nbsp; ${'⭐'.repeat(stars)}`;
  document.getElementById('pauseBtn').classList.add('hidden'); document.getElementById('musicBtn').classList.add('hidden'); document.getElementById('sfxBtn').classList.add('hidden'); stopMusic();
...
function loseLevel(){
  state='lose';
  stopMusic();
  document.getElementById('loseStats').textContent = `Score: ${player.score} — Probeer het nog eens!`;
  document.getElementById('pauseBtn').classList.add('hidden'); document.getElementById('musicBtn').classList.add('hidden'); document.getElementById('sfxBtn').classList.add('hidden'); stopMusic();
  ...
  for (const e of level.entities.enemies){
    ...
    switch(e.type){
      ...
      case 'hopper': {
        ...
        if (e.vy===0 || e.onG){ }
        const floorY = snapToGround(e.x, e.y);
        if (floorY!=null && e.y>=floorY-1){ e.y=floorY; e.vy=-11; e.x += e.dir*(Math.sin(e.t)*0+2); }
        e.y += e.vy*0.5;
        break;
      }
      case 'flyer': {
...
function collideEnemyPlayer(e){
  ...
  if (px<ex+ew && px+pw>ex && py<ey+eh && py+ph>ey){
    if (player.starTime>0){
      defeatEnemy(e, true); return;
    }
    if (e.type==='piranha' || e.type==='turret' || e.type==='bullet'){
      hurtByEnemy(e); return;
    }
...
function hurtByEnemy(e){
  if (e.shell && e.dir===0){ e.dir = player.x<e.x?1:-1; e.speedShell=6; return; }
...
function updateBoss(){
  ...
  // phase determined by remaining HP (fight escalates as boss gets weaker)
  const hpRatio = b.hp / b.maxHp;
  b.phase = hpRatio > 0.66 ? 1 : hpRatio > 0.33 ? 2 : 3;

  if (b.stunT>0){
    // brief vulnerable stagger after being hit
    b.y = (level.groundY-4)*TILE + Math.sin(b.t*6)*4;
  } else {
    // movement pattern: patrol the arena, speed scales with phase
    ...
    // attack patterns, faster cadence each phase
    ...
    if (b.attackT > cadence){
      ...
      if (b.phase>=2 && roll<0.4){
        ...
        // jump-smash toward player's side
        b.vyBoss = -13;
        b.jumping = true;
      } else {
        // charge dash
  ...
  if (b.jumping){
    b.vyBoss = (b.vyBoss||0) + GRAVITY*0.7;
    b.y += b.vyBoss;
  ...
  if (px<b.x+bw && px+pw>b.x && py<b.y+bh && py+ph>b.y){
    ...
    else if (player.vy>0 && py+ph-b.y<20 && b.hitCooldown===0){
      damageBoss(b); player.vy=-9;
    } else if (b.hitCooldown===0) {
      hurtByEnemy({shell:false});
    }
  }
}
function damageBoss(b){
  ...
  if (b.hp<=0){
    b.alive=false;
    floatingTexts.push({x:b.x,y:b.y,text:'BOSS VERSLAGEN!',life:90,vy:-0.5});
    playSfx('bossDefeat');
...
function draw(){
  ...
  // parallax hills (depth layer 1, slow)
  ctx.fillStyle = th.hills;
  const hillOff = -(camX*0.25)%400;
  for (let i=-1;i<4;i++){
    const bx = hillOff + i*400;
    ctx.beginPath();
    ctx.ellipse(bx+200, H-40, 260, 120, 0, Math.PI, 0);
    ctx.fill();
  }
  // clouds
  if (th.cloud){
...
function drawArenaDecor(arena, theme){
  if (!arena) return;
  // torches/crystals on the arena pillars for atmosphere
  ...
  ctx.restore();
  // ambient banner strip marking the arena entrance
...
function drawBoss(b){
  ...
  const scale = 1 + (b.maxHp>14?0.15:0); // slightly bigger for later, tougher bosses
  ctx.scale(scale,scale);

  // glow aura that intensifies with phase
  ctx.save();
  ...
  // spikes on back for later phases
  if (b.phase>=2){
  ...
  // hp bar (bigger, with phase label)
  ...
  ctx.fillText(`FASE ${b.phase}`, b.x+40, b.y-42);
}
...
function drawEnemy(e){
  ...
  switch(e.type){
    case 'goomba':
      ...
      break;
    case 'koopa':
      ctx.fillStyle = e.shell? '#3ea832':'#3ea832';
...
function drawPlayer(){
  ...
  // shadow ellipse for pseudo-3d grounding
...
function playSfx(kind){
  ...
  try{
    switch(kind){
      ...
      case 'doublejump': tone(523,0,0.09,'square',0.07,880); break;
      ...
      case 'powerup':
        [523,659,784,1047].forEach((f,i)=> tone(f,i*0.07,0.14,'square',0.09));
        break;
      case 'shieldup':
        [392,494,587].forEach((f,i)=> tone(f,i*0.06,0.12,'sine',0.08));
        break;
      case 'hurt': tone(220,0,0.14,'sawtooth',0.1,110); break;
      case 'death':
        tone(392,0,0.16,'sawtooth',0.1,196);
        tone(261,0.16,0.16,'sawtooth',0.09,110);
        tone(174,0.32,0.3,'sawtooth',0.08,60);
        break;
      case 'win':
        [523,659,784,1047,1319].forEach((f,i)=> tone(f,i*0.11,0.22,'square',0.09));
        break;
      case 'bossHit': tone(150,0,0.1,'square',0.12,70); tone(80,0.05,0.15,'sawtooth',0.1,40); break;
      case 'bossDefeat':
        [220,196,164,131,98].forEach((f,i)=> tone(f,i*0.16,0.3,'sawtooth',0.11));
        break;
      case 'bossRoar':
        tone(80,0,0.4,'sawtooth',0.14,40); tone(60,0.1,0.5,'square',0.1,30);
        break;
      case 'break': tone(140,0,0.08,'square',0.08); tone(100,0.04,0.1,'square',0.06); break;
...
// Simple generative background music: a short looping bass+lead motif per theme
const MUSIC_SCALES = [
  [261,293,329,392,440], // groen: vrolijk majeur
  [220,246,261,311,349], // grot: mineur, donkerder
  [277,311,370,415,466], // woestijn: exotisch
  [246,277,311,370,415], // ijs: kalm mineur
  [196,220,233,277,311], // vulkaan: gespannen
];
let musicStepIdx = 0;
function startMusic(worldIdx){
  ...
  function schedule(){
    ...
    // bass note every step
    const bassNote = musicStepIdx%4===0 ? root : root*Math.pow(2,1/12*[0,0,7,0][musicStepIdx%4]/1);
    ...
    // lead melody note (semi-random walk across scale for variety)
    if (musicStepIdx%2===0){
